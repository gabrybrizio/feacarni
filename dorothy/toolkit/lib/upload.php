...
class Upload {
  ...
  protected function move() {
    ...
    if($source['error'] !== 0) {
      ...
      switch($source['error']) {
        ...
        case UPLOAD_ERR_EXTENSION:
          $this->fail(static::ERROR_WRONG_FILE_TYPE);
        default:
    ...
    if($this->options['acceptType']) {
      $file = new Media($source['name']);
      $tempFile = new Media($source['tmp_name']);
      $ext = $file->extension();

      // eccezione per immgini jpeg
      $ext = str_replace('jpeg', 'jpg', str::lower($ext));

      $extFromMime = f::mimeToExtension($tempFile->mime());

      // se l'estensione del nome del file non coincide con l'estensione ricavata dal mime type contenuto nel file
      if ($ext !== $extFromMime) {
        $this->fail(static::ERROR_WRONG_FILE_TYPE);
      }

      // l'estensione deve esser tra quelle permesse dal type indicato
      $foundExt = false;
      if(in_array($extFromMime, f::$types[$this->options['acceptType']])) {
        $foundExt  = true;
      }
  ...
  protected function messages() {
    return array(
      static::ERROR_MISSING_FILE        => 'Il file è mancante',
      static::ERROR_MISSING_TMP_DIR     => 'La cartella /tmp non è presente sul server',
      static::ERROR_FAILED_UPLOAD       => 'Il caricamento è fallito',
      static::ERROR_PARTIAL_UPLOAD      => 'Il file è stato caricato parzialmente',
      static::ERROR_UNALLOWED_OVERWRITE => 'Il file esiste già e non può esser sovrascritto',
      static::ERROR_MAX_SIZE            => 'Il file è troppo grande. La dimensione massima consentita è di ' . f::niceSize($this->maxSize()),
      static::ERROR_MOVE_FAILED         => 'Il file non può essere spostato',
      static::ERROR_UNACCEPTED          => 'Il file non è stato accettato dal server',
      static::ERROR_WRONG_FILE_TYPE     => 'Il formato del file non è corretto. Le estensioni accettate sono: ' . implode(', ', f::$types[$this->options['acceptType']])
    );
  }
