...
class A {
  ...
  public static function v($array, $key, $escape = true){
    $currentLang = lang::current();
    $specificLanguageKey = "{$key}_$currentLang";

    if (array_key_exists($specificLanguageKey, $array)) {
      $key = $specificLanguageKey;
    }

    if ($escape){
        return escape::html(static::get($array, $key));
    }else{
  ...
  /**
   * Restituisce gli item che non sono in entrambi gli array
   * Attenzione! Non è ricorsivo, controlla solo il primo livello
   *
   * @param array|null $a
   * @param array|null $b
   * @return array
