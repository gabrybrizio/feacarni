...
class Thumb extends Obj {
  ...
  public static function src($image, $w = null){
    if($image != ''){
        
        // Costruisco il path dell'immagine
        if ($w !== null){
            $thumbsDir = $image->dir() . '/thumbs/';
            $thumbsFolder = New folder($thumbsDir);
            $thumbsFolder->create();
            
            $imagePATH = $thumbsFolder . $image->name() . '__w' . $w . '.' . $image->extension();
        }else{
            $imagePATH = $image->root();
        }
        
        $imageURL = str_replace('\\', '/', $imagePATH); // Windows backslash
        $imageURL = str::after($imageURL, 'app-data/');
        $imageURL = util::dataUrl() . $imageURL;		
        
        // Se l'immagine non esiste la creo
        if(!file_exists($imagePATH)){
            $thumb = new \abeautifulsite\simpleimage($image->root());
            $thumb->auto_orient();
            $thumb->fit_to_width($w)->save($imagePATH); 
        }		
